...
@section('content')
    <div class="container mt-5">
        <h2>Редактирование страницы всех новостей</h2>
        <form class="form-horizontal" method="POST" action="{{ route('admin.allNews.edit') }}">
            @csrf
            ...
            <div class="form-group mt-3">
                ...
                <div class="col-sm-10">
                    <input type="text" class="form-control" placeholder="H2" name="h2"
                        value="{{ old('h2', optional($settings)['h2']) }}">
                </div>
                @if ($errors->has('h2'))
                    <span class="text-danger">{{ $errors->first('h2') }}</span>
                @endif
            </div>
            <div class="form-group mt-3">
                <label class="control-label col-sm-2">SEO текст конец страницы:</label>
                <div class="col-sm-10">
                    <input type="text" class="form-control" placeholder="SEO текст" name="seo_text2"
                        value="{{ old('seo_text2', optional($settings)['seo_text2']) }}">
                </div>
                @if ($errors->has('seo_text2'))
                    <span class="text-danger">{{ $errors->first('seo_text2') }}</span>
                @endif
            </div>
            {{-- <div class="form-group mt-3">
